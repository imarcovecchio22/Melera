...
- **Promos por cantidad** (precio por escalón): 5 o más frascos a $6.000 c/u, 10 o más a $5.500 c/u (editables en `/admin/stock` → "Precio y stock"). El total lo calcula siempre el servidor (`src/lib/precios.ts`); se muestran en `/producto` (atajos 1 · 5 · 10), el checkout, la home, `/consultas`, el chat y las respuestas de Instagram (`$PROMOS`)
- Panel `/admin` protegido: pedidos (estado, detalle, origen), precio, promos y stock, consultas (link directo a ig.me / mailto, marcar respondida, archivar) y **logs**. Fechas en hora de Argentina
...
- **Tipos:** presentación, dato curioso, producto y **promo** (muestra las promos de "Precio y stock" con los precios del momento en que se genera; Gemini solo escribe título, CTA y caption).
